...
4. Generate final page sections
   ...
   Use staged generation checks. After generating the first 2-3 slices, concatenate a small preview and inspect it before continuing. Continue to the remaining slices only if the preview shows a shared visual system, correct product identity, varied copy structures, and acceptable top/bottom continuity. If the early preview fails, revise the blueprint, 1:3 master, or slice prompts before generating the rest.
